...
public class ConnectedAreaTests
{
	...
	{
		var area = new ConnectedArea(new Position[] { new(11, 11), new(12, 11), new(12, 12) });
		...
		trimmed
			.Should()
			.BeEquivalentTo(
				new Position[] { new(0 + border, 0 + border), new(1 + border, 0 + border), new(1 + border, 1 + border) }
			);
	}
	...
	{
		var area = new ConnectedArea(new Position[] { new(0, 0), new(1, 0), new(1, 1) });

		var (trimmed, dimensions) = area.TrimExcess(2);

		dimensions.Should().Be(new Size(6, 6));
		trimmed.Should().BeEquivalentTo(new Position[] { new(2, 2), new(3, 2), new(3, 3) });
	}
}
