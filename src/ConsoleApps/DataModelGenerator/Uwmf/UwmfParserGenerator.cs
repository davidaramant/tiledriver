...
public static class UwmfParserGenerator
{
	public static void WriteToPath(string basePath)
	{
		...
		using var stream = File.CreateText(Path.Combine(basePath, "UwmfParser.Generated.cs"));
		...
		var includes = new[]
		{
			...
			"Tiledriver.FormatModels.Uwmf",
		};

		output
			.WriteHeader("Tiledriver.FormatModels.Uwmf.Reading", includes, enableNullables: true)
			.Line($"[GeneratedCode(\"{CurrentLibraryInfo.Name}\", \"{CurrentLibraryInfo.Version}\")]")
			.Line("public sealed partial class UwmfParser")
			.OpenParen();
	...
	{
		var collectionProperties = topLevelBlock
			.Properties.OfType<CollectionProperty>()
			.Where(property => property.Name != "planeMap")
			.ToArray();

		output
			.Line("private void AddGeneratedParsedBlock(")
			.IncreaseIndent()
		...
		{
			var block = UwmfDefinitions.Blocks.Single(b =>
	...
	private static string CreateParseAssignmentExpression(ScalarProperty property) =>
		property switch
		{
			...
			TextureProperty => "ParseTextureFieldValue()",
	...
	private static bool IsRequiredProperty(ScalarProperty property) => property.DefaultString == null;

	private static string CreateRequiredCheck(Block block, ScalarProperty property, int index) =>
		$"if (!HasFlag(seenFields, {CreateSeenMask(block, property, index)})) throw MissingRequiredField(\"{property.FormatName}\");";
