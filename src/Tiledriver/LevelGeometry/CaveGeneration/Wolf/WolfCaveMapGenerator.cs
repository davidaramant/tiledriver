...
using Tiledriver.GameInfo.Wolf3D;
...
namespace Tiledriver.LevelGeometry.CaveGeneration.Wolf;
...
public static class WolfCaveMapGenerator
{
	...
	public static MapData Create(int seed, string texturePrefix, TextureQueue textureQueue)
	{
		...
		var caveBoard = new CellBoard(new Size(128, 128))
			.Fill(random, probabilityAlive: 0.5)
			.MakeBorderAlive(thickness: 3)
			.GenerateStandardCave();

		var (caveArea, size) = ConnectedAreaAnalyzer
		...
		var alternateFloor = new CellBoard(new Size(size.Width + 1, size.Height + 1))
			.Fill(random, probabilityAlive: 0.5)
			.RunGenerations(6);
		var alternateCeiling = new CellBoard(new Size(size.Width + 1, size.Height + 1))
			.Fill(random, probabilityAlive: 0.5)
			.RunGenerations(6);
		...
		var (floorLighting, ceilingLighting) = LightTracer.Trace(size, p => !caveArea.Contains(p), lightRange, lights);

		var (planeMap, sectors, tiles) = CreateGeometry(
			size,
			caveArea,
			alternateFloor,
			alternateCeiling,
	...
	)
	{
		...
		string GetTextureName(Corners corners, int light, bool isEastWest = false)
		{
			...
			{
				patches.Add(new Patch($"{texturePrefix}{(int)corners:D2}", 0, 0));
			}
		...
		Corners GetCorners(CellBoard board, Position pos) =>
			Corner.CreateFromUpperLeft(upperLeft: pos, on: p => board[p] == CellType.Dead);

		Corners GetSideCorners(Position left, Position right) =>
			Corner.Create(
		...
		var sectorSequence = new ModelSequence<SectorDescription, Sector>(description => new Sector(
		...
		var tileSequence = new ModelSequence<TileDescription, Tile>(description => new Tile(
		...
		for (int y = 0; y < size.Height; y++)
		{
			for (int x = 0; x < size.Width; x++)
			{
				...
				int sectorId = sectorSequence.GetIndex(
				...
				planeMap.Set(x, y, tile: tileId, sector: sectorId, zone: 0);
			}
		}
