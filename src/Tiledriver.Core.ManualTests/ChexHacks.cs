...
public sealed partial class ChexHacks
{
	...
	public async Task FixFlats()
	{
		...
		var modFlatsDirectory = Path.Combine(ChexProjectPath, "flats");

		var newNames = usedFlats.Select((name, i) => (name, i)).ToDictionary(x => x.name, x => $"CQ{x.i:00}");
		newNames["F_SKY1"] = "F_SKY1";

		var outputFlatDir = Path.Combine(ProjectPath, "flats", "chex");
		if (!Directory.Exists(outputFlatDir))
			Directory.CreateDirectory(outputFlatDir);

		foreach (var flat in usedFlats)
		{
			File.Copy(
				Path.Combine(modFlatsDirectory, $"{flat}.png"),
				Path.Combine(outputFlatDir, $"{newNames[flat]}.png")
			);
		}
		...
		var updatedMap = mapData with
		{
			Sectors =
			[
				.. mapData.Sectors.Select(s =>
					s with
					{
						TextureCeiling = newNames[s.TextureCeiling.Name],
						TextureFloor = newNames[s.TextureFloor.Name],
					}
				),
			],
		};
	...
	public async Task FixTextures()
	{
		...
		var chexTex1 = ParseTextures(await File.ReadAllTextAsync(Path.Combine(ChexProjectPath, "TEXTURES.txt")))
		...
		var existingChexPatchLookup = Directory
			.GetFiles(Path.Combine(ChexProjectPath, "patches"))
			.ToDictionary(path => Path.GetFileNameWithoutExtension(path)!.ToUpper(), path => Path.GetFileName(path)!);

		var chexPatchNewNameLookup = chexPatches
			.Select((name, index) => (name, index))
			.ToDictionary(x => x.name, x => $"CQP{x.index:00}");

		var outputPatchDir = Path.Combine(ProjectPath, "patches", "chex");
		if (!Directory.Exists(outputPatchDir))
			Directory.CreateDirectory(outputPatchDir);

		foreach (var chexPatch in chexPatches)
		{
			File.Copy(
				Path.Combine(ChexProjectPath, "patches", existingChexPatchLookup[chexPatch]),
				Path.Combine(
					outputPatchDir,
					chexPatchNewNameLookup[chexPatch] + Path.GetExtension(existingChexPatchLookup[chexPatch])
				),
				overwrite: true
			);
		}
		...
		var updatedMap = mapData with
		{
			...
			[
				.. mapData.SideDefs.Select(sd =>
					sd with
					{
						TextureBottom = ReplaceTexture(sd.TextureBottom, textureNameTransform),
						TextureMiddle = ReplaceTexture(sd.TextureMiddle, textureNameTransform),
						TextureTop = ReplaceTexture(sd.TextureTop, textureNameTransform),
					}
				),
			],
		};
		...
		static Texture ReplaceTexture(Texture tex, IReadOnlyDictionary<string, string> texLookup) =>
			texLookup.TryGetValue(tex.Name, out var newName) ? newName : tex;

		static WallTexture LookupDefinition(
			IReadOnlyDictionary<string, WallTexture> tex,
			IReadOnlyDictionary<string, WallTexture> tex2,
			string name
		)
		{
			if (tex.TryGetValue(name, out var def))
			{
				return def;
			}
	...
	private static async Task WriteTextureDefinitionsAsync(string path, IEnumerable<WallTexture> newTextures)
	{
	...
	private static IReadOnlyList<WallTexture> GetDoom2Textures() => ParseTextures(Doom2TexturesDefinition);

	private static IReadOnlyList<WallTexture> ParseTextures(string definition)
	{
